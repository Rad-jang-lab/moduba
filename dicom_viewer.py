...
class DicomViewer:
    def __init__(self, root: tk.Tk) -> None:
        ...
        self.measurement_mode_var = tk.StringVar(value="off")
        self.temp_measurements: list[dict[str, Any]] = []
        self._active_measurement_start: tuple[int, int] | None = None
        self._active_measurement_current: tuple[int, int] | None = None
    ...
    def _build_ui(self) -> None:
        ...
        ttk.Label(top, text="측정").pack(side="left", padx=(12, 0))
        ttk.Radiobutton(
            top,
            text="Off",
            variable=self.measurement_mode_var,
            value="off",
            command=self._on_measurement_mode_changed,
        ...
        ttk.Button(top, text="측정 지우기", command=self.clear_temporary_measurements).pack(side="left", padx=(8, 0))
        ttk.Checkbutton(
            top,
            text="Compare Mode",
    ...
    def _on_measurement_mode_changed(self) -> None:
        self._active_measurement_start = None
        self._active_measurement_current = None
        self._draw_single_view_measurements()
    ...
    def _start_pan(self, event: tk.Event) -> None:
        if not self.frames:
            return
        if self.view_mode == "single" and self.measurement_mode_var.get() in {"roi", "line"}:
    ...
    def _update_pan(self, event: tk.Event) -> None:
        if not self.frames:
            return
        if self.view_mode == "single" and self._active_measurement_start is not None:
    ...
    def _end_pan(self, _event: tk.Event) -> None:
        if self.view_mode == "single" and self._active_measurement_start is not None:
            self._finish_temporary_measurement()
            return
        if self.view_mode == "single":
            self._draw_single_view_overlays()
        return
    ...
    def _draw_single_view_measurements(self) -> None:
        self.canvas.delete("measurement")
        if self.view_mode != "single" or self.photo_image is None or not self.frames:
            return
        frame = np.asarray(self.frames[self.current_frame])
        geometry = self._get_single_view_geometry(frame)
        if geometry is None:
            return

        for item in self.temp_measurements:
            self._draw_measurement_item(item, geometry)

        if self._active_measurement_start is not None and self._active_measurement_current is not None:
            preview_item = {
                "mode": self.measurement_mode_var.get(),
                "start": self._active_measurement_start,
                "end": self._active_measurement_current,
                "preview": True,
            }
            self._draw_measurement_item(preview_item, geometry)

    def _draw_measurement_item(self, item: dict[str, Any], geometry: dict[str, float]) -> None:
        mode = item.get("mode")
        start = item.get("start")
        end = item.get("end")
        if mode not in {"roi", "line"} or start is None or end is None:
            return
        x0, y0 = start
        x1, y1 = end
        p0 = self._map_frame_pixel_to_canvas(x0, y0, geometry)
        p1 = self._map_frame_pixel_to_canvas(x1, y1, geometry)
        if p0 is None or p1 is None:
            return
        dash = (3, 2) if item.get("preview") else None
        if mode == "roi":
            left = min(p0[0], p1[0])
            top = min(p0[1], p1[1])
            right = max(p0[0], p1[0])
    ...
    def _get_single_view_geometry(self, frame: np.ndarray) -> dict[str, float] | None:
        frame_array = np.asarray(frame)
        if frame_array.ndim == 2:
            frame_height, frame_width = frame_array.shape
        elif frame_array.ndim == 3:
            ...
            return None
        if frame_width <= 0 or frame_height <= 0 or self.photo_image is None:
            return None
        ...
        return {
            ...
            "image_left": float(image_left),
            "image_top": float(image_top),
        }

    @staticmethod
    def _map_frame_pixel_to_canvas(pixel_x: int, pixel_y: int, geometry: dict[str, float]) -> tuple[float, float] | None:
        ...
        if not (0 <= pixel_x < frame_width and 0 <= pixel_y < frame_height):
            return None
        x_ratio = (pixel_x + 0.5) / frame_width
        y_ratio = (pixel_y + 0.5) / frame_height
        canvas_x = geometry["image_left"] + x_ratio * geometry["display_width"]
        canvas_y = geometry["image_top"] + y_ratio * geometry["display_height"]
    ...
    def _start_temporary_measurement(self, event: tk.Event) -> None:
        if self.measurement_mode_var.get() not in {"roi", "line"}:
            return
        if not self.frames or self.photo_image is None:
            return
        frame = np.asarray(self.frames[self.current_frame])
        if frame.ndim == 2:
            frame_height, frame_width = frame.shape
        ...
        if coords is None:
            return
        self._active_measurement_start = coords
        self._active_measurement_current = coords
        self._draw_single_view_measurements()

    def _update_temporary_measurement(self, event: tk.Event) -> None:
        if self._active_measurement_start is None:
            return
        if not self.frames or self.photo_image is None:
            return
        frame = np.asarray(self.frames[self.current_frame])
    ...
    def _finish_temporary_measurement(self) -> None:
        if self._active_measurement_start is None or self._active_measurement_current is None:
            self._active_measurement_start = None
            self._active_measurement_current = None
            self._draw_single_view_measurements()
            return
        ...
        mode = self.measurement_mode_var.get()
        if mode in {"roi", "line"} and start != end:
            self.temp_measurements.append({"mode": mode, "start": start, "end": end})
